...
def pc_decoder(disp_net):
    ...
    # collect occlusion contour from hit result (on given xz-plane)
...
if __name__ == '__main__':
    ...
    imgL_path_list = sorted(Path(data_dir/'imL').glob('*.png'), key=lambda v: int(v.stem))

    print('* Restoring model')
    disp_net = DispNet('test')
    disp_net.model.load_weights('.results_stereosv/weights-067.h5')
    tf_pred = tf.function(partial(pc_decoder(disp_net).call, training=False))

    for imgL_path in imgL_path_list:
        imgR_path = (data_dir/'imR'/imgL_path.stem).with_suffix('.png')

        imgL = utils.imread(str(imgL_path))
        imgR = utils.imread(str(imgR_path))
        imgL_fit, imgR_fit = utils.resize_image_pairs(imgL, imgR, (opt.img_width, opt.img_height), np.float32)

        t0 = time.time()
        p_abs = tf_pred([imgL_fit[None], imgR_fit[None], nK[None], baseline[None]])
        p_abs = p_abs[0].numpy()
        print('* p_abs:')
        for i, v in enumerate(p_abs):
            print(',' if i > 0 else '', f'{v:.6f}f', end=' ')
        print()
        gmap, eoa = gen_gmap(p_abs, nK)
        cte, ye = get_visual_odometry(gmap)
        t1 = time.time()
        print("*", imgL_path.stem, "elspaed:", t1 - t0, "cte:", cte, "ye:", ye)

        # display minimap
        minimap = get_minimap(gmap, cte, ye)
        if utils.imshow(minimap, name='gmap', norm='gray', wait=True) == 27:
            break
