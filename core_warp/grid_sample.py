...
def grid_sample(image, grid, name='grid_sample'):
    with tf.name_scope(name):
        ...
        with tf.name_scope('interpolate_bilinear'):
            ...
            x0m, y0m = x0c == x0, y0c == y0
            x1m, y1m = x1c == x1, y1c == y1

            batch_offsets = tf.range(B) * H * W
            batch_offsets = batch_offsets[:,None]
            flattened_img = tf.reshape(image, (-1, C))

            def gather_with_zero_padding(row, col, row_mask, col_mask, name):
                with tf.name_scope('gather_' + name):
                    mask = tf.cast(tf.math.logical_and(row_mask, col_mask), tf.float32)
...
if __name__ == '__main__':
    ...
    gx, gy = np.meshgrid(np.linspace(-2,1,5), np.linspace(-1,1,7))
